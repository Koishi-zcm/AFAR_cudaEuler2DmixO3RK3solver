...
:
    ...
    recalcBasis_(mesh.schemes().subDict("CUDA").lookupOrDefault("recalcBasis", true)),
    ...
    maxCompactLocalBlockStencilSize_(0),
    stencilType_(mesh.schemes().subDict("CUDA").lookup("stencilType"))
{
    ...
    setDeviceFieldData(
        ...
        maxStencilSize_, maxCompactStencilSize_,
        maxLocalBlockStencilSize_, maxCompactLocalBlockStencilSize_
    );
    ...
    delete[] hostPtr_.compactStencilSize;
    delete[] hostPtr_.RBFbasis;
    delete[] hostPtr_.extendStencilSize;
    delete[] hostPtr_.compactExtendStencilSize;
...
basicCUDAsolver::~basicCUDAsolver()
{
    ...
    delete[] hostPtr_.FD;
    delete[] hostPtr_.shockIndicator;
...
void basicCUDAsolver::initStencilData()
{
    ...
    Foam::List<Foam::List<int>> cellFaces_orig(mesh.nCells());
    Foam::List<Foam::labelList> cellCells(mesh.nCells());
    for (int faceI = 0; faceI < facesNum_; ++faceI)
    {
        const int own = owner_orig[faceI];
        const int nei = neighbour_orig[faceI];
        hostPtr_.neighbour[2*faceI] = own;
        hostPtr_.neighbour[2*faceI+1] = nei;

        const int ownfaceI = faceI+1;
        const int neifaceI = -(faceI+1);
        cellFaces_orig[own].append(ownfaceI);
        cellFaces_orig[nei].append(neifaceI);

        cellCells[own].append(nei);
        cellCells[nei].append(own);
    }
    ...
    for (int i = 0; i < totalBoundaryFacesNum_; ++i)
    {
        ...
        const int ownfaceI = faceI+1;
        const int boundaryCelli = i + cellsNum_;
        cellFaces_orig[celli].append(ownfaceI);
        cellCells[celli].append(boundaryCelli);
    }
    ...
    int* globalStencil = new int[maxStencilSize_*cellsNum_];
    for (int celli = 0; celli < cellsNum_; ++celli)
    {
    ...
    if (recalcBasis_)
    {
        ...
        for (int celli = 0; celli < cellsNum_; ++celli)
        {
            ...
            if (troubleMatrix[celli]) troubleMatrixNum++;
        }
        std::cout << std::endl;

        if (existTroubleMatrix) Foam::Info << "Warning: Singular transformMatrix exist! troubleMatrixNum = " << troubleMatrixNum << Foam::endl;

        cellGaussPoints.clear();
        cellGaussWeights.clear();

        const Foam::surfaceVectorField& faceCentre = mesh.Cf();
        hostPtr_.RBFbasis = new float[2*(maxStencilSize_+1)*facesNum_];
        Foam::List<Foam::List<Foam::scalar>> basisOwnList(facesNum_);
        Foam::List<Foam::List<Foam::scalar>> basisNeiList(facesNum_);
        for (int faceI = 0; faceI < facesNum_; ++faceI)
        {
            ...
            if (troubleMatrix[own])
            {
                ...
                Foam::scalarField RBFbasisOwn(stencilSizeOwn, 0.0);
                RBFbasisOwn[0] = 1.0;
                basisOwnList[faceI] = RBFbasisOwn;
            }
            else
            {
            ...
            if (troubleMatrix[nei])
            {
                ...
                Foam::scalarField RBFbasisNei(stencilSizeNei, 0.0);
                RBFbasisNei[0] = 1.0;
                basisNeiList[faceI] = RBFbasisNei;
            }
            else
            {
        ...
        refDistance.clear();
        transformMatrixList.clear();
        troubleMatrix.clear();

        Foam::Info << "writing basis List..." << Foam::endl;

        Foam::IOList<Foam::List<Foam::scalar>> IObasisOwnList(
            Foam::IOobject(
                "basisOwnList",
                mesh.time().system(),
                mesh,
        ...
        Foam::IOList<Foam::List<Foam::scalar>> IObasisNeiList(
            Foam::IOobject(
                "basisNeiList",
                mesh.time().system(),
                mesh,
        ...
        IObasisOwnList.write();
        IObasisNeiList.write();
    }
    else
    {
        Foam::Info << "reading RBFbasis from file..." << Foam::endl;
        ...
        for (int faceI = 0; faceI < facesNum_; ++faceI)
        {
            const Foam::List<Foam::scalar>& basisOwn = IObasisOwnList[faceI];
            const Foam::List<Foam::scalar>& basisNei = IObasisNeiList[faceI];

            for (int i = 0; i < basisOwn.size(); ++i)
            {
                hostPtr_.RBFbasis[2*(maxStencilSize_+1)*faceI + i] = basisOwn[i];
            }

            for (int i = 0; i < basisNei.size(); ++i)
            {
                hostPtr_.RBFbasis[2*(maxStencilSize_+1)*faceI + i + (maxStencilSize_+1)] = basisNei[i];
            }
        }
    }
    ...
    hostPtr_.compactExtendStencilSize = new uint16_t[gridDim];
    int maxValidLocalBlockStencilSize = (maxSharedMemSize - sizeof(double)*blockDim*4 - sizeof(float)*blockDim*2 - sizeof(uint16_t)*blockDim*maxStencilSize_ - sizeof(uint8_t)*blockDim)/(sizeof(int8_t));
    int* globalExtendCells = new int[maxValidLocalBlockStencilSize*gridDim];
    for (int blockIdx = 0; blockIdx < gridDim; ++blockIdx)
    {
        ...
        hostPtr_.compactExtendStencilSize[blockIdx] = extendCells.size();
        maxCompactLocalBlockStencilSize_ = Foam::max(extendCells.size() + blockDim, maxCompactLocalBlockStencilSize_);
        ...
        for (int threadIdx = 0; threadIdx < blockDim; ++threadIdx)
        {
            ...
            for (int i = hostPtr_.compactStencilSize[celli]; i < hostPtr_.stencilSize[celli]; ++i)
            {
                const int extendID = globalStencil[maxStencilSize_*celli + i];
                bool isInExtendStencil = extendID < blockIdx*blockDim || extendID > blockDim-1 + blockIdx*blockDim || extendID >= cellsNum_;
                if (isInExtendStencil)
    ...
    Foam::Info << "maxCompactLocalBlockStencilSize : " << maxCompactLocalBlockStencilSize_ << Foam::endl;
    ...
    for (int blockIdx = 0; blockIdx < gridDim; ++blockIdx)
    {
        ...
        for (int threadIdx = 0; threadIdx < blockDim; ++threadIdx)
        {
            ...
            {
                std::cout << std::unitbuf 
                    << "changing globalStencil into localStencil data (step 3)...[" 
                    << std::to_string(celli+1) << '/' 
                    << std::to_string(cellsNum_) << ']' << '\r';
            }
            if (celli >= cellsNum_) break;
    ...
    size_t sharedMemSize1 = sizeof(double)*maxCompactLocalBlockStencilSize_*2
...
void basicCUDAsolver::writeField(const Foam::Time& runTime)
{
    ...
    for (int i = 0; i < cellsNum_; ++i)
    {
        ...
        shockIndicator[i] = hostPtr_.shockIndicator[2*i+1];
    }
